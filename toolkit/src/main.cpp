...
SupervisedLearner* getLearner(string model, Rand& r)
{
	if (model.compare("baseline") == 0)
		...
		return new NearestNeighbor(r);
	else if (model.compare("cluster") == 0)
		return new Clustering(r);
	else
...
void doit(ArgParser& parser)
{
	...
	if ( evaluation.compare ( "training" ) == 0 )
	{
		...
		double accuracy = 0;//learner->measureAccuracy(trainFeatures, trainLabels, &stats);
